...
class SOLUTION:
    ...
    def create_body(self):
        ...
        pyrosim.Send_Cube(name="Torso", pos=[0, 0, 1], size=[0.5, 0.5, 1])  # root link

        # SHOULDERS
        pyrosim.Send_Cube(name="BackLeg", pos=[0, -0.25, 0.3], size=[0.2,0.2,0.2])
        pyrosim.Send_Cube(name="FrontLeg", pos=[0, 0.25, 0.3], size=[0.2,0.2,0.2])

        # UPPER LEGS
        pyrosim.Send_Cube(name="LeftLeg", pos=[0, 0.05, -1], size=[0.2, 0.25, 0.5])
        pyrosim.Send_Cube(name="RightLeg", pos=[0, -0.05, -1], size=[0.2, 0.25, 0.5])

        # LOWER LEGS
        pyrosim.Send_Cube(name="LeftLowerLeg",pos=[0, 0.05, -0.5], size=[0.2, 0.25, 0.5])
        pyrosim.Send_Cube(name="RightLowerLeg", pos=[0, -0.05, -0.5], size=[0.2, 0.25, 0.5])

        # ARMS
        pyrosim.Send_Cube(name="FrontLowerLeg", pos=[0, 0, -0.1], size=[0.2, 0.25, 0.75])
        pyrosim.Send_Cube(name="BackLowerLeg", pos=[0, 0, -0.1], size=[0.2, 0.25, 0.75])

        # SHOULDER JOINTS
        pyrosim.Send_Joint(name="Torso_BackLeg", parent="Torso", child="BackLeg", type="revolute",
                           position=[0, -0.05, 1], jointAxis = "1 0 0")
        pyrosim.Send_Joint(name="Torso_FrontLeg", parent="Torso", child="FrontLeg", type="revolute",
                           position=[0, 0.05, 1], jointAxis = "1 0 0")

        # UPPER LEG JOINTS
        pyrosim.Send_Joint(name="Torso_LeftLeg", parent="Torso", child="LeftLeg", type="revolute",
                           position=[0, 0.05, 1], jointAxis="0 1 0")
        pyrosim.Send_Joint(name="Torso_RightLeg", parent="Torso", child="RightLeg", type="revolute",
                           position=[0, -0.05, 1], jointAxis="0 1 0")

        # ARM JOINTS
        pyrosim.Send_Joint(name="FrontLeg_FrontLowerLeg", parent="FrontLeg", child="FrontLowerLeg", type="revolute",
                           position=[0, 0.4, 0], jointAxis="1 0 0")
        ...
        pyrosim.Send_Joint(name="LeftLeg_LeftLowerLeg", parent="LeftLeg", child="LeftLowerLeg", type="revolute",
                           position=[0, 0.05 -0.5], jointAxis="0 1 0")
