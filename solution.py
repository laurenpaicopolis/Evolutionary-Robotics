...
class SOLUTION:
    ...
    def create_body(self):
        ...
        pyrosim.Send_Cube(name="Torso", pos=[0, 0, 1], size=[length, width, 1.5])  # root link

        pyrosim.Send_Cube(name="BackLeg", pos=[0, -0.25, 0.5], size=[0.2,0.2,0.2])
        pyrosim.Send_Cube(name="FrontLeg", pos=[0, 0.25, 0.5], size=[0.2,0.2,0.2])
        #pyrosim.Send_Cube(name="LeftLeg", pos=[-0.5, 0, 0], size=[1, 0.2, 0.2])
        #pyrosim.Send_Cube(name="RightLeg", pos=[0.5, 0, 0], size=[1, 0.2, 0.2])
        pyrosim.Send_Cube(name="FrontLowerLeg", pos=[0, 0, 0.1], size=[0.2, 0.25, 1])
        pyrosim.Send_Cube(name="BackLowerLeg", pos=[0, 0, 0.1], size=[0.2, 0.25, 1])
        #pyrosim.Send_Cube(name="LeftLowerLeg", pos=[0, 0, -0.5], size=[0.2, 0.2, 1])
        #pyrosim.Send_Cube(name="RightLowerLeg", pos=[0, 0, -0.5], size=[0.2, 0.2, 1])

        pyrosim.Send_Joint(name="Torso_BackLeg", parent="Torso", child="BackLeg", type="revolute",
                           position=[0, -0.2, 1], jointAxis = "1 0 0")
